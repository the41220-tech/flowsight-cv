...
- ✅ **Phase A — BEV anomaly detectors** (2026-06-22, committed): `flowsight/anomaly/` = radial-divergence (flee), fast-approach (pre-attack), emergency-void (fall), geofence + `TerrorComposite` + `AnomalyMonitor`; runner `experiments/anomaly_run.py`; tests 5/5. Ran on real UMN (divergence 545/774, fast 253, terror 496 — over-firing, thresholds need per-scene baseline tuning; max_div 4.24 at the t=233 s panic). 4 of 5 patterns are lightweight BEV signals; violence (E) is the only video model.
- ✅ **DepthGroundCalibrator** (`geometry/calibration.py`) + wired into `absolute_alarm_run` (`--depth-npz --fov-deg`). Metric depth → ground-plane → ray-plane metric (no survey). Unit-tested (<2% distance error). **OOM bug fixed** (near-horizon points → giant grid) via t-clamp + robust `metric_bounds`.
- ✅ **Unattended harness** (`finetune/violence_train.py` resilient YOLOv8-cls + `prepare_rwf.py` + `BACKGROUND_RUN.md`); roadmap/gap doc `docs/roadmap_gap_2026-06-22.md`.

**⚠️ Open finding — absolute MAGNITUDE not yet calibrated on uncalibrated footage:** depth-scale on UMN gave peakP **486/s²** (pedestrian-scale 20; physical ~1–2) — near-horizon local m/px inflates velocity variance and FOV=65° is a guess. The geometry is correct (unit test) but absolute magnitude needs surveyed points OR a calibrated FOV → validate on **WILDTRACK ground-GT (Phase F)**. The *relative* spatial pressure map + the 0.02 threshold (once calibrated) are the moat; the alarm FIRES correctly, the absolute number is not yet trustworthy.

**Next (priority order):**
1. **Launch Phase B — violence classifier (unattended):** push the `prepare_rwf` `_find_label` fix, then on Colab Pro (L4, tab open + `caffeinate`; Pro has NO background execution): `prepare_rwf.py` (streaming, all clips) → `violence_train.py` (30 ep, Drive ckpt, resumable). RWF-2000 format = `['avi' bytes, '__key__' label, '__url__']`.
2. **Calibration accuracy:** clamp near-horizon velocity/area + near-field restriction in `DepthGroundCalibrator`; validate absolute magnitude on WILDTRACK.
3. **Precision refinement:** lift the over-sensitive alarms (pressure 680/774; divergence 545/774) via duration-gate/hysteresis + per-scene baseline.
